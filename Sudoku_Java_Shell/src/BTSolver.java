...
public class BTSolver
{
	...
	public Map.Entry<HashMap<Variable,Integer>,Boolean> norvigCheck ( )
	{
		...
		// Part II -> traverse through all constraints and assign the variable whose domain value is not being used by any other neighboring value.

		List<Variable> variableList = network.getVariables();
		HashMap<Variable,Integer> ansMap = new HashMap<Variable,Integer>();	
		for(Variable v: variableList)
		{
			if(!v.isAssigned())
			{
				// Map to maintain domain value to list of variables which can have that value
				HashMap<Integer,ArrayList<Variable>> map = new HashMap<Integer,ArrayList<Variable>>();
				List<Variable> neighbors = network.getNeighborsOfVariable(v);
				// Populate map with current variable domain values
				for(int value: v.getDomain())
				{
					...
					{
						map.put(value,new ArrayList<Variable>());
						map.get(value).add(v);
					}
				}

				//Populate the neighbor domain values in the map
				for(Variable neighbor: neighbors)
				{
					if(!neighbor.isAssigned())
					{
						for(int value: neighbor.getDomain())
						{
							if(map.containsKey(value))
								map.get(value).add(neighbor);
							else
							{
								map.put(value,new ArrayList<Variable>());
								map.get(value).add(neighbor);
				...
				// Traverse the map and see if any value has only one variable to assign to
				for(int key: map.keySet())
				{
					if(map.get(key).size() == 1)
					{
						trail.push(map.get(key).get(0));
						map.get(key).get(0).assignValue(key);
						ansMap.put(map.get(key).get(0),key);
					}
				}
			}
		}
		...
		if(network.isConsistent())
			return Pair.of(ansMap,true);
                return Pair.of(ansMap,false);
	}
